...
- [ ] 汇总条文字随状态变化
- [ ] 每行状态徽章带 spinner（处理中态）/ 绿色徽章（完成）
- [ ] 完成后 chunk_count 正确（与 chunks.jsonl 中相同 doc_stem 的行数一致）
...
- [ ] 失败行显著区分（红色徽章 + 错误链接）
- [ ] 错误信息可读（不是 `unknown error`）
- [ ] 「重试」后该行回到 `pending → parsing → …`
...
- [ ] chunks tab 至少能看到第一块的 markdown 渲染（不是裸 HTML）
- [ ] merged tab 拼接结果与 KB 的 chunks.jsonl 顺序一致
- [ ] preview tab 不可点（disabled）
...
- [ ] 重启后被卡死的文档行变为「失败」+ 错误信息
- [ ] 「重试」按钮可用，点击后能进入新的 ingest 流程
